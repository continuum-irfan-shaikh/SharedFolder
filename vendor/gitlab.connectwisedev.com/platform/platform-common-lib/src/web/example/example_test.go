...
import (
	...
	"gitlab.connectwisedev.com/platform/platform-common-lib/src/web/mock"
)
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				...
				mockServer.EXPECT().GetRouter().Return(mockRouter)
				mockRouter.EXPECT().AddFunc(gomock.Any(), gomock.Any(), gomock.Any())
				mockRouter.EXPECT().AddHandle(gomock.Any(), gomock.Any(), gomock.Any())
				mockRouter.EXPECT().Use(gomock.Any(), gomock.Any(), gomock.Any())
