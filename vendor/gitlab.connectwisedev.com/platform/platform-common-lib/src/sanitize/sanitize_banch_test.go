	...
	for i := 0; i < b.N; i++ {
		for _, tt := range tests {
			if got := Identifier(tt.args.s); got != tt.want {
				b.Errorf("Identifier() : test %v = %v, want %v", tt.name, got, tt.want)
			}
		}
	}
}
	...
	for i := 0; i < b.N; i++ {
		for _, tt := range tests {
			if got := HTML(tt.args.s); got != tt.want {
				b.Errorf("HTML() : Test %v = %v, want %v", tt.name, got, tt.want)
			}
		}
	}
}

func BenchmarkHTMLAllowing(b *testing.B) {
	type args struct {
		s    string
		args [][]string
	}
	...
	for i := 0; i < b.N; i++ {
		for _, tt := range tests {
			...
			if (err != nil) != tt.wantErr {
				b.Errorf("HTMLAllowing() : Test %v error = %v, wantErr %v", tt.name, err, tt.wantErr)
				return
			}
			if got != tt.want {
				b.Errorf("HTMLAllowing() : Test %v = %v, want %v", tt.name, got, tt.want)
			}
		}
	}
}
	...
	for i := 0; i < b.N; i++ {
		for _, tt := range tests {
			if got := Name(tt.args.s); got != tt.want {
				b.Errorf("Name() : Test %v = %v, want %v", tt.name, got, tt.want)
			}
		}
	}
}
	...
	for i := 0; i < b.N; i++ {
		for _, tt := range tests {
			if got := FileName(tt.args.s); got != tt.want {
				b.Errorf("FileName() : Test %v = %v, want %v", tt.name, got, tt.want)
			}
		}
	}
}
	...
	for i := 0; i < b.N; i++ {
		for _, tt := range tests {
			if got := Path(tt.args.s); got != tt.want {
				b.Errorf("Path() : Test %v = %v, want %v", tt.name, got, tt.want)
			}
		}
	}
}
	...
	for i := 0; i < b.N; i++ {
		for _, tt := range tests {
			if got := Number(tt.args.s); got != tt.want {
				b.Errorf("Number() Test %v = %v, want %v", tt.name, got, tt.want)
			}
		}
	}
}
