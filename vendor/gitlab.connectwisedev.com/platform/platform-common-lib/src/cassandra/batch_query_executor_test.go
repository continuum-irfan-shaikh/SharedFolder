...
func TestGetBatchQueryExecutor(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: &batchQueryExecutorImpl{batchConnection: &batchConnection{Connection: connection{conf: &DbConfig{Hosts: []string{"Sever"}, Keyspace: "test",
				...
				batch: &gocql.Batch{}}, batch: &gocql.Batch{}},
			...
			setup: func() {
				session.EXPECT().NewBatch(gomock.Any()).Return(&gocql.Batch{}).Times(2)
