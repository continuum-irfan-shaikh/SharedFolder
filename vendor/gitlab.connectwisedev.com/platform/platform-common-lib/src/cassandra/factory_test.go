...
func TestFactoryImpl_GetDbConnector(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{cfg: &DbConfig{Hosts: []string{"Sever"}, Keyspace: "test"}}, setup: func() {
				cql.CreateSession = func(cluster *gocql.ClusterConfig) (cql.Session, error) { return msession, errors.New("Error") }
				msession.EXPECT().Closed().Return(true)
			},
		},
		{
			name: "Session Creation Success", wantErr: false,
			args: args{cfg: &DbConfig{Hosts: []string{"Sever"}, Keyspace: "test",
...
func TestFactoryImpl_GetBatchDbConnector(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func() {
				msession.EXPECT().NewBatch(gomock.Any()).Return(&gocql.Batch{})
				msession.EXPECT().Closed().Return(true)
				cql.CreateSession = func(cluster *gocql.ClusterConfig) (cql.Session, error) { return msession, nil }
			},
		},
