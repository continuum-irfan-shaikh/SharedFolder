...
import (
	...
	"gitlab.connectwisedev.com/platform/platform-common-lib/src/validate/is"
...
func TestNewDbConnection(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: &connection{conf: &DbConfig{Hosts: []string{"Sever"}, Keyspace: "test",
			...
			args: args{conf: &DbConfig{Hosts: []string{"Sever"}, Keyspace: "test",
				NumConn: 10, TimeoutMillisecond: time.Millisecond * 10, CircuitBreaker: &circuit.Config{
					Enabled: true, TimeoutInSecond: 1, MaxConcurrentRequests: 15000,
					ErrorPercentThreshold: 25, RequestVolumeThreshold: 500, SleepWindowInSecond: 10,
				}, CommandName: "Database-Command1", ValidErrors: []string{"Error1"}}}, setup: func() {
