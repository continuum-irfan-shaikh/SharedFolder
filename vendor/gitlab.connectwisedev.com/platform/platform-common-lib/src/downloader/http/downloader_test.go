...
import (
	...
	cMock "gitlab.connectwisedev.com/platform/platform-common-lib/src/webClient/mock"
)
...
func Test_serviceImpl_Download(t *testing.T) {
	...
	clientMock3.EXPECT().Create().Times(2)
	clientMock3.EXPECT().Do(gomock.Any()).Return(resp3, nil).Times(2)
	clientMock3.EXPECT().SetCheckRedirect(gomock.Any()).Times(2)
...
func Test_DownloadUsingGateway(t *testing.T) {
	...
	clientMock := cMock.NewMockHTTPClientService(ctrl)
	clientMock.EXPECT().Create().Times(4)
	clientMock.EXPECT().Do(gomock.Any()).Return(nil, errors.New("Error"))
	clientMock.EXPECT().Do(gomock.Any()).Return(resp, nil).Times(4)
	clientMock.EXPECT().SetCheckRedirect(gomock.Any()).Times(4)
