...
# $TypeOfInstaller = 'Installer' # string [mandatory]
# $PackageName = 'adobeflash.exe' # string [mandatory]
# $TypeOfApplication = 'exe' # string [mandatory]
# $LocationType = 'network' # string [mandatory]

# $http = "dcmdwld.itsupport247.net/FileZilla_3.8.0_setup.exe"  # string [mandatory]
# $network = "\\10.2.19.25\e$\Antivirus Setups\Prateek"  # string [mandatory]
# $local = "c:\Users\Admin\AppData\Local\Temp\"  # string [mandatory]
# $UninstallationPath = "C:\Program Files\Google\Chrome\Application\74.0.3729.131\Installer\setup.exe" 
# $UninstallationPath = "C:\Users\Admin\AppData\Local\Temp\adobeflash.exe"  # string [mandatory]
# $UninstallationParameter = 'MsiExec.exe /qn /quiet /X{C197BC08-3D82-4651-8886-E68C21578A38}'
# $UninstallationParameter = '-uninstall'
# $Username = "administrator"  # string [mandatory]
# $Password = "Grt@2018"  # string [mandatory]
# $MD5CheckSum = '60F53389A99B94E547136EE93B5C9378' # string
# $InstallationParameter = '-install' # string

# $PerformPreCheck = $true   # Boolean
# $PreCheckType = 'Check if file exists'   # String
# $PreCheckAction = 'If file exists then Continue'   # String
# $PreCheckFileName = 'C:\temp\CaptureScreenshot.ps1'   # String
...
# $PreCheckProgram = 'Google Chrome' # string

# $PerformPostCheck = $true   # Boolean
# $PostCheckType = 'Check if Add / Remove program exists'   # String
# $PostCheckAction = 'If the program exists then mark success'   # String
# $PostCheckFileName = 'C:\prateek\FTPDown2\csetup.exe'   # String
# $PostCheckVersion = '7.0.4.3'   # String
# $PostCheckKey = 'HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\DB Browser for SQLite'   # String
# $PostCheckValue = 'nomodify'   # String
# $PostCheckData = '1'   # String
# $PostCheckProgram = 'Adobe Flash Player 11 Plugin' # string
...
function Test-FileVersion ($Path, [System.Version]$UserSpecifiedVersion) { 
    ...
    $FileVersion = (Get-Item $Path -ErrorAction SilentlyContinue).versioninfo.fileversionraw
...
Function Test-AddRemoveProgram ($Program) {
    ...
    $product = Get-ChildItem $Registry -ErrorAction 'SilentlyContinue' | Get-ItemProperty | Where-Object Displayname -eq $Program
    return [bool] $product
}
#endregion Functions
    ...
    if ($PerformPreCheck) {
        ...
        Switch ($PreCheckType) {
            ...
            'Check if Add / Remove program exists' {
                ...
                Switch ($PreCheckAction) {
                    'If the program exists then mark continue' { if ($ProgramExists) { continue } else { Write-Output "Aborting because $Name condition was not met `'$PreCheckAction`'"; return } }
                    'If the program does not exists then mark continue' { if (!$ProgramExists) { continue } else { Write-Output "Aborting because $Name condition was not met `'$PreCheckAction`'"; return } }
                    'If the program exists then mark abort' { if ($ProgramExists) { Write-Output "Aborting because $Name condition was met `'$PreCheckAction`'"; return } else { continue } }
                    'If the program does not exists then mark abort' { if (!$ProgramExists) { Write-Output "Aborting because $Name condition was met `'$PreCheckAction`'"; return } else { continue } }
                }
                break;
            }
        }
