<#
  .Script
  Enable always show for icons and notification.
  .Author
  Nirav Sachora.
  .Version
  2.0 and above.
...
if($Details -ne $null)
    ...
    if($Details -contains 'EnableAutoTray')
                        {
                        Set-ItemProperty $path -Name 'EnableAutoTray' -Value 0
                        }
                        else
                        {
                        New-ItemProperty -Path $path -Name 'EnableAutoTray' -PropertyType "DWord" -Value 0 | Out-Null
                        }
	...
	if((Get-ItemProperty $path -Name 'EnableAutoTray'  | Select -ExpandProperty EnableAutoTray) -eq 0){return $true} else {return $false}
...
Write-Output "Always show for icons and notifications has been enabled for below users on this system`n"
Write-Output $Users
