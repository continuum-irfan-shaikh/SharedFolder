...
Try {
    ...
    $URL = 'http://sdupdt.itsupport247.net/tpupdate/Windows10Upgrade.exe'
    ...
    if ($SystemDriveFreeSpace -lt 32) {
        Write-Output "Prerequisite check failed : Minimum 32GB free space is needed on system drive"
        Break;
    }
    ...
    While(1){
        ...
        # check log status
        $UpdateSuccesful = (Get-Content $LogFile | Where-Object {$_ -like "*State manager get called back with state PostInstallState, hr 0x0*"}) -as [Bool]
        if ($UpdateSuccesful) {
            if($UpdateSuccesful){
                Write-Output "Installation of version 1903 feature update completed successfully. `nPlease reboot the system for changes to take affect, or system will reboot after 30 minutes."
            }
            else{
