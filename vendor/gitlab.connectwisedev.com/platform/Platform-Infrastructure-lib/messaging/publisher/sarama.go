...
func reconnect(transaction string, cfg *Config) {
	...
	for producerType, v := range existingProducer {
		log.Info(transaction, "Closing old %s Kafka Producer", producerType)
		err := v.cleanExisting(transaction)
		if err != nil {
			log.Error(transaction, "kafka.connection.close.failed", "Error in closing %s Kafka producer %v", producerType, err)
		}
	}
}
